...
public class DebugWorm implements BiConsumer<AbstractCondition, Object> {
    ...
    public void accept(AbstractCondition condition, Object result) {
        DebugResult debugResult = debugResultMap.computeIfAbsent(condition.getId(), key -> new DebugResult().setConditionId(key));
        debugResult.setConditionName(condition.getName());
